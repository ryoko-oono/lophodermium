...
lambertiana = here::here("Data", "lambertiana.kml")
...
monticola = here::here("Data", "monticola.kml")
...
lambertiana_points<- as.data.frame(read.csv(here::here("Data","Locations_lambertiana.csv"), 
                                            check.names = F, header=TRUE, stringsAsFactors = T,
                                            strip.white=TRUE, na.strings=c("NA", "na",
                                                                           "nan", "inf", "", ".")))
monticola_points<- as.data.frame(read.csv(here::here("Data", "Locations_monticola.csv"), 
                                          check.names = F, header=TRUE, stringsAsFactors = T,
                                          strip.white=TRUE, na.strings=c("NA", "na",
                                                                         "nan", "inf", "", ".")))

lophodermium_species<- as.data.frame(read.csv(here::here("Data", "Lophodermium_species2.csv"), 
                                          check.names = F, header=TRUE, stringsAsFactors = T,
                                          strip.white=TRUE, na.strings=c("NA", "na",
                                                                         "nan", "inf", "", ".")))
...
ui <- fluidPage(
                # Application title
                titlePanel(HTML("Open-access real-time ecological and evolutionary analyses of pine-specific <em>Lophodermium</em>")),
                h2("Introduction"),
                "This is a working draft of a dynamic collection database for the pine-associated genus", 
                em("Lophodermium."), 

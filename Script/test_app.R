...
species_list <- list("lambertiana.kml", "monticola.kml")
...
ui <- fluidPage(
  ...
  em("Lophodermium"), "is distributed worldwide but most well-studied in regions with high densities of pines (ca. 115 species). 
  ...
  mainPanel( 
    ...
    absolutePanel(top = 60, left = 20, 
                  checkboxGroupInput(inputId = "pine_species", "Species ranges to see:",
                                     choiceNames =
                                       list("Pinus lambertiana","Pinus monticola"),
                                     choiceValues =
                                       list("lambertiana.kml", "monticola.kml"),
                                     selected = NULL
                  ),
  ...
  fluidRow(
    ...
    column(4,
           selectInput("ascocarp",
                       "Ascocarp position:",
                       c("All",
                         unique(as.character(lophodermium_species$Ascocarp_position))))
    )
  ),
...
server <- function(input, output) {
   ...
   observe({
        ...
        if(is.null(input$pine_species)){
          proxy %>%  clearShapes()
          output$checkBoxText <- renderText({length(input$pine_species)})
          } 
        else{
          for (i in 1:length(input$pine_species)){
            if (i == 1){
                        new_proxy <- proxy %>% 
                          registerPlugin(kmlPlugin) %>%
                          addPolygons(data = input$pine_species[i], # CHANGE cases by variableplot
                                     color = "green",
                                      dashArray = "3",
                                      fillOpacity = 0.5) 
               } else{
...
#
#    if (input$pine_species %in% species_list){
#proxy
            #      for(i in 1:length(input$pine_species)){
   #         if (i == 1){
  #          new_proxy <- proxy %>% 
  ...
  #     })
     ...
     output$table <- DT::renderDataTable(DT::datatable({
       ...
       if (input$subgenus != "All") {
         data <- data[data$Host_subgenus == input$subgenus,]
       }
